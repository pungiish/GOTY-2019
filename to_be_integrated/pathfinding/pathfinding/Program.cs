...
{ 
    ...
    class Program
    {
        ...
        {
            int x, y, startx, starty, moveRange, destx, desty;
#if INTERACTIVE
            Console.WriteLine("1. vrstica: zacetna tocka figure(x, y), tocke premika");
            ...
            for(int i = 0; i < y; ++i)
            {
                ...
                {
                    map[j, i] = new Tile();
                    map[j, i].Weight = int.Parse(line[j]);   
                }
            }

            warrior.FindPossibleMoves(startx, starty, map);
#if INTERACTIVE
            ...
            LinkedList<Tuple<int, int>> path = warrior.GetPath(destx, desty, map);
            if (path.Count == 0)
            {
